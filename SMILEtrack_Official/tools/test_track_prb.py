...
def detect():
    ...
    if not exists(opt.txt_dir):
        mkdir(opt.txt_dir)
    #res_file = opt.project + '/' + opt.name + ".txt"
    res_file = opt.txt_dir + '/'  + opt.name + ".txt"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='MOT17', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--weights', nargs='+', type=str, default='PRB-FPN6-MSP.pt', help='model.pt path(s)')
    ...
    parser.add_argument('--txt-dir', dest="txt_dir", type=str, default='prb_track_results', help='dir to save result txt files')
